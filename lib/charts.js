...
(function (global) {
  ...
  // Give every SVG a programmatic name and a complete text alternative. The
  // values remain visible in the graph, while screen readers can consume the
  // same information without interpreting positions or colours.
  function describeChart(svg, opts, series) {
    const id = `jkd-chart-${++chartId}`;
    const titleId = `${id}-title`;
    const descId = `${id}-desc`;
    const title = opts.title || "Kanban metrics chart";
    const unit = opts.unit ? ` ${opts.unit}` : "";
    const parts = (series || []).map((s) => {
      const label = s.label || "Values";
      return `${label}: ${opts.labels.map((x, i) => `${x} ${s.values[i] == null ? "not available" : s.values[i]}${unit}`).join(", ")}.`;
    });
    svg.setAttribute("role", "img");
    svg.setAttribute("aria-labelledby", `${titleId} ${descId}`);
    svg.appendChild(el("title", { id: titleId }, title));
    svg.appendChild(el("desc", { id: descId }, parts.join(" ")));
  }
  ...
  function barChart(container, opts) {
    ...
    const W = Math.max(
      560,
      opts.minBandWidth
        ? m.left + m.right + opts.labels.length * opts.minBandWidth
        : 560
    );
    ...
    if (opts.minBandWidth) svg.setAttribute("width", W);
    describeChart(svg, opts, [{ label: opts.seriesLabel || "Values", values: opts.values }]);
  ...
  function groupedBarChart(container, opts) {
    ...
    const selectedIdx = opts.selectedIndex != null ? opts.selectedIndex : -1;
    const band = iw / n;
    ...
    for (let i = 0; i < n; i++) {
      ...
      // The selected historical week and the true current week use distinct
      // redundant encodings (outlined blue vs warm filled band).
      if (i === selectedIdx && i !== curIdx) {
        svg.appendChild(el("rect", {
          x: m.left + band * i + 2, y: m.top,
          width: band - 4, height: ih, rx: 6, class: "band-selected",
        }));
      }
      if (i === curIdx) {
        svg.appendChild(el("rect", {
      ...
      svg.appendChild(el("text", {
        ...
        class: "axis-label" + (isCur ? " axis-label-current" : "") +
          (isSelected && !isCur ? " axis-label-selected" : ""),
        "text-anchor": "middle",
      }, opts.labels[i]));
    }
    svg.appendChild(el("line", {
  ...
  function lineChart(container, opts) {
    ...
    if (opts.minBandWidth) svg.setAttribute("width", W);
    describeChart(svg, opts, [{ label: opts.seriesLabel || "Values", values: raw }]);
    ...
    raw.forEach((v, i) => {
      ...
      svg.appendChild(el("text", {
        ...
        class: "axis-label" + (isCurrent ? " axis-label-current" : "") +
          (isSelected && !isCurrent ? " axis-label-selected" : ""),
        "text-anchor": "middle",
      }, opts.labels[i]));
      if (v == null || isNaN(v)) return;
      svg.appendChild(el("circle", {
        cx: cx(i), cy: cy(v), r: isCurrent || isSelected ? 5.5 : 4,
        class: "dot" + (isCurrent ? " dot-current" : "") +
          (isSelected && !isCurrent ? " dot-selected" : ""),
      }));
      svg.appendChild(el("text", {
        x: cx(i), y: cy(v) - 10,
