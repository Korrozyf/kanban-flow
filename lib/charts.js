/* Kanban Flow — mini bibliothèque de graphiques SVG (sans dépendance) */
(function (global) {
  "use strict";
  ...
  // Graphique en barres. opts: { labels, values, unit, highlightLast, color }
  ...
  // Graphique en barres groupées (plusieurs séries côte à côte par semaine).
  // opts: { labels, series: [{ values, className }], currentIndex }
  // La semaine en cours est surlignée par un bandeau jaune en arrière-plan.
  function groupedBarChart(container, opts) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Bandeau de fond pour la semaine en cours.
      if (i === curIdx) {
        svg.appendChild(el("rect", {
      ...
      series.forEach((s, si) => {
        ...
        const x = gx + bw * si;
        const y = m.top + ih - h;
        // La semaine en cours utilise une couleur chaude dédiée par série.
  ...
  // Courbe. opts: { labels, values, unit }
  function lineChart(container, opts) {
    ...
    // Segments reliant les points valides ; le segment aboutissant à la
    // semaine en cours est tracé en jaune pointillé (donnée incomplète).
    let prev = null;
    raw.forEach((v, i) => {
