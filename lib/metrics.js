...
(function (global) {
  ...
  // Maximum number of previous complete weeks displayed.
  ...
  /* Builds the week window from the "macrocycle start date" (first week). The
   * current week is always the last one; up to MAX_PREV_WEEKS previous complete
   * weeks are shown, bounded by the macrocycle's first week.
   * Returns:
   *  - future: true            → the first week is after the current week
   *  - weeks[]                 → { start, end, label, current, complete }
   *  - currentIndex            → index of the current week (last one)
   *  - completeCount           → number of complete weeks shown
   *  - hasComparison           → at least 1 previous week to compare against
   */
  /* Parses the "macrocycle start date" as a LOCAL date at midnight.
  ...
  function buildMacrocycleWeeks(macroStart, now) {
    ...
    // Without a valid macrocycle date: fall back to the current week + MAX_PREV_WEEKS
    // previous complete weeks (default behaviour).
  ...
  function computeIssueDates(issue, project, clampStart) {
    ...
    // Clamped to the macrocycle start: if a reference date (creation for lead
    // time, first move to in-progress for cycle time) is earlier than the
    // macrocycle start, it is pulled forward to that date. This means time
    // elapsed before the macrocycle start is never counted.
    if (clampStart) {
  ...
  async function analyzeProject(client, project, cfg, onProgress) {
    // Window driven by the macrocycle start date (first week).
    const win = buildMacrocycleWeeks(cfg.macrocycleStart);
    if (win.future) {
      return {
        future: true,
    ...
    const n = weeks.length;
    const windowStart = weeks[0].start;
    // Lead/cycle time clamp date = the chosen macrocycle start date (falls back
    // to the window's 1st week if no valid date is set).
    const clampStart =
