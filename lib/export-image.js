 ...
 *
 * One preparation step is mandatory before every capture, and a second one is
 * opt-in:
 ...
 *  2. openAllDetails(): when the user explicitly chooses to include ticket
 *     lists, collapsed <details> are opened for the duration of the capture and
 *     then restored. The privacy-preserving default captures them collapsed.
 */
(function (global) {
  "use strict";
   ...
   *   openDetails include collapsed ticket lists (default false)
  ...
  async function exportNode(node, opts) {
    ...
    const restoreDetails = o.openDetails ? openAllDetails(node) : () => {};
