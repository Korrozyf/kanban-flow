...
(function (global) {
  ...
  async function checkForUpdate(opts) {
    ...
    // Cache failures too: opening several extension pages while GitHub is down
    // must not turn one outage into repeated requests (or exhaust the shared
    // anonymous rate limit). An explicit user check still bypasses the cache.
    if (!force && fresh) return buildState(cached);
